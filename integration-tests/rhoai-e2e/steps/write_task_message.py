...
from suite.test_output_pass_rate import gate_pass_rate
...
def _test_output_counts(obj: dict[str, object]) -> tuple[int, int, int]:
    suites = obj.get("suites")
    if isinstance(suites, list) and suites:
        passed = sum(int(s.get("passed", 0)) for s in suites if isinstance(s, dict))
        failed = sum(int(s.get("failed", 0)) for s in suites if isinstance(s, dict))
        skipped = sum(int(s.get("skipped", 0)) for s in suites if isinstance(s, dict))
        return passed, failed, skipped
    passed = int(obj.get("successes", obj.get("passed", 0)) or 0)
    failed = int(obj.get("failures", 0) or 0)
    skipped = int(obj.get("skipped", 0) or 0)
    return passed, failed, skipped


def _warning_head_label(results: dict[str, str], *, task_label: str) -> str:
    """Head line for WARNING TEST_OUTPUT (partial component pass)."""
    label = "Partial pass"
    raw = results.get("TEST_OUTPUT", "").strip()
    if raw.lstrip().startswith("{"):
        ...
        if isinstance(obj, dict):
            passed, failed, skipped = _test_output_counts(obj)
            rate = gate_pass_rate(passed=passed, failed=failed, skipped=skipped)
            if rate is not None:
                label = f"{rate * 100:.0f}% pass rate"
    if task_label:
        return f"{task_label}: {label}"
    return label
...
def build_task_message(*, pipeline_task: str = "", results: dict[str, str] | None = None) -> str:
    ...
    if term_msg or step:
        ...
        if task_label == "test-finalize":
            ...
            head = _warning_head_label(sibling, task_label=task_label)
            msg = f"{head}\n{hint}" if hint else head
    ...
    if drift:
        ...
        if drift_clause not in msg:
            ...
            if "Failed" in first_line or "% pass rate" in first_line or "Partial pass" in first_line:
