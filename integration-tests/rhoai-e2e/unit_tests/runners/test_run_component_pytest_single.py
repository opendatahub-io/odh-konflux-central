...
def test_pooled_external_smoke_prep_runs_when_cluster_prep_done(
    ...
    monkeypatch.setenv("COMPONENT_TEST_COMPONENT_ID", "kuberay")
    monkeypatch.setenv("PRODUCT", "rhoai")
    monkeypatch.setenv("CLUSTER_SOURCE", "rhoai-e2e-kubeconfig-nmanos-konflux1-nmanos")
    monkeypatch.delenv("TEST_OUTPUT_PATH", raising=False)
...
def test_ensure_rhoai_e2e_on_pythonpath(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("PYTHONPATH", raising=False)
    run_component_pytest._ensure_rhoai_e2e_on_pythonpath()
    root = str(run_component_pytest._RHOAI_E2E_ROOT)
