...
class ResolveRhoaiFbcForItsApplicationTest(unittest.TestCase):
    def _runner(self, *, run_its: str = "rhoai-e2e-ephc-ocp421") -> RhoaiE2ERunner:
        parser = make_parser()
...
class RhoaiAppVersionKeyTest(unittest.TestCase):
    def test_parses_major_minor(self) -> None:
        self.assertEqual(
            RhoaiE2ERunner._rhoai_app_version_key("rhoai-v3-5-ea-2"),
            (3, 5),
        )

    def test_ignores_non_numeric_segments(self) -> None:
        self.assertEqual(
            RhoaiE2ERunner._rhoai_app_version_key("rhoai-v3-4-foo"),
            (3, 4),
        )
