...
_TASKS_DIR = _RHOAI_E2E / "tekton" / "tasks"
...
def test_task_message_wiring_present() -> None:
    ...
    _assert_task_message_wiring(pipe, label="rhoai-e2e-pipeline.yaml")
...
""",
    ...
    assert msg.startswith("test-maas-billing: 75% pass rate.\n")
...
def test_build_task_message_failure_from_test_output_without_termination(
    ...
    assert msg.startswith("test-maas-billing: 94% pass rate.\n")
...
def test_publish_results_pipeline_results_reference_publish_task() -> None:
    ...
    assert plr_results["RHOAI_E2E_SUMMARY_TEST_OUTPUT"] == "$(finally.publish-results.results.TESTS_SUMMARY)"
...
def test_publish_results_task_results_fit_tekton_budget() -> None:
    ...
    oversized_summary = format_run_summary_block(
        pipeline_run_name="rhoai-e2e-ods-qe-psi-07-bvt-smoke-nmanos-dfzbn",
