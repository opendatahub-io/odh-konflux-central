...
class RunnerTriggerMixin:
    ...
    def _preview_snapshot_json(self, odh_overrides: bool) -> str:
        """Build inline SNAPSHOT JSON before PipelineRun create (for naming / trigger params)."""
        if getattr(self, "_trigger_snapshot_spec", None):
            return json.dumps(self._trigger_snapshot_spec)
        if getattr(self, "snapshot_file", None):
            try:
                return self._build_snapshot_json(odh_overrides)
            except AppError:
                return ""
        return ""

    def _catalog_version_for_naming(
        self,
        odh_overrides: bool,
    ...
    def _render_its_for_trigger(self, odh_overrides: bool) -> None:
        ...
        fbc_version = self._catalog_version_for_naming(
            odh_overrides,
            rhoai_version_param=version_display["RHOAI_VERSION"],
        )
        generate_prefix = build_rhoai_e2e_generate_prefix(
            product=self.args.product,
            version=fbc_version,
            cluster_source=cluster_source,
            cluster_label=self._cluster_label_for_naming(cluster_source),
    ...
    def _build_snapshot_json(self, odh_overrides: bool) -> str:
        ...
        snap_dict: dict[str, Any] = {"application": self.args.app, "components": [comp]}
        snap_meta = getattr(self, "_fbc_source_snapshot_meta", None)
        if isinstance(snap_meta, dict) and snap_meta:
            snap_dict["metadata"] = snap_meta
