...
def load_install_context() -> InstallContext:
    ...
    operator_install_dir_s = iav.require_env("OLMINSTALL_DIR")
    ...
    operator_install_dir = Path(operator_install_dir_s)
    if not operator_install_dir.is_dir():
        iav.fail(f"❌ OLMINSTALL_DIR is not a directory: {operator_install_dir_s}")
    install_script = operator_install_dir / "install-operator.sh"
    ...
    return InstallContext(
        ...
        operator_install_dir=operator_install_dir,
...
def phase_operator_install_subscription(ctx: InstallContext) -> str:
    iav.patch_oc_wait_sh(ctx.operator_install_dir, ctx.operator_namespace)
    iav.patch_oc_wait_install_plan_timeout(ctx.operator_install_dir)
    try:
        csv_wait_retries = max(60, int(float(os.environ.get("OPERATOR_CSV_WAIT_SEC", "1200")) // 10))
    except ValueError:
        csv_wait_retries = 120
    iav.patch_oc_wait_csv_timeout(ctx.operator_install_dir, retries=csv_wait_retries)
    manifest_path = iav.resolve_olminstall_manifest(ctx.operator_install_dir, ctx.operator_name)
    ...
    unpack_timeout = int(
        os.environ.get(
            "OLM_BUNDLE_UNPACK_TIMEOUT_SEC",
            str(default_olm_bundle_unpack_timeout_sec()),
        )
    )
    ...
    print(
        f"Running rhoai-e2e (./install-operator.sh {ctx.operator_name} "
...
def phase_post_install_dsc(ctx: InstallContext) -> None:
    ...
    if product == "rhoai" and not gateway_config_ready():
        print(
...
def print_install_plan(ctx: InstallContext) -> None:
    product = os.environ.get("PRODUCT", "").strip().lower()
    product_label = {"rhoai": "RHOAI"}.get(product, product or "operator")
    print("")
    print("=========================================")
