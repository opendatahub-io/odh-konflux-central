...
def main() -> int:
    ...
    operator_install_dir_s = require_env("OLMINSTALL_DIR")
    ...
    if not operator_install_dir.is_dir():
        fail(f"❌ OLMINSTALL_DIR is not a directory: {operator_install_dir_s}")
    ...
    patch_oc_wait_sh(operator_install_dir, operator_namespace)
    manifest_path = resolve_olminstall_manifest(operator_install_dir, operator_name)
    patch_manifest_namespace(manifest_path, operator_namespace)

    print(
        f"Running rhoai-e2e (./install-operator.sh {operator_name} {update_channel} {catalog_name})..."
    )
    r_install = subprocess.run(
        ["./install-operator.sh", operator_name, update_channel, catalog_name],
        cwd=operator_install_dir,
        timeout=7200,
    )
    if r_install.returncode != 0:
        print("❌ rhoai-e2e install-operator.sh failed")
