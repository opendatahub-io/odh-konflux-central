#!/usr/bin/env python3
"""
Tekton step: read TESTS param + rhoai-e2e-tests-config.yaml, write RUN_SMOKE / RUN_BVT / RUN_TIER1 result files.
...
def main() -> int:
    ...
    if not repo_root:
        ...
        return 1
    root = Path(repo_root)
    cfg = root / "integration-tests" / "rhoai-e2e" / "rhoai-e2e-tests-config.yaml"
    ...
    try:
        ...
        print(
            ...
            f"integration-tests/rhoai-e2e/rhoai-e2e-tests-config.yaml: {exc}",
            file=sys.stderr,
        )
        return 1
