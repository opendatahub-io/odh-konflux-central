...
def rc_fallback_tags(csv_version: str) -> list[str]:
    m = _RC_RE.match(csv_version)
    if not m:
        return []
    major, minor, rc = m.group(1), m.group(2), int(m.group(3))
    tags: list[str] = []
    tags.append(f"{major}.{minor}-rc.{rc}")
    tags.append(f"{major}.{minor}rc{rc}")
    tags.append(f"{major}.{minor}")
    return tags
...
def resolve_versioned_image(repo: str, csv_version: str) -> str:
    ...
    if m:
        ...
        rc_match = _RC_RE.match(csv_version)
        if rc_match:
            tags = rc_fallback_tags(csv_version)
        else:
            m = _MAJOR_MINOR_RE.match(csv_version)
            if m:
    ...
    if _EA_RE.match(csv_version):
        ...
        ea_match = _EA_RE.match(csv_version)
        if ea_match:
            same_minor_ga = f"{ea_match.group(1)}.{ea_match.group(2)}"
            candidate = f"{repo}:{same_minor_ga}"
            if _tag_exists(repo, same_minor_ga):
                print(f"EA CSV {csv_version}: using same-minor GA image {candidate}")
                return candidate
            print(f"Same-minor GA tag not found: {candidate}")
