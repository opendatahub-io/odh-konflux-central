...
def _distributed_workloads_ready_condition_for_version(operator_version: str) -> str:
...
def _resolve_smoke_ready_condition(
    smoke_id: str,
    *,
    ...
    if smoke_id == "distributed_workloads":
        version = (operator_version or "").strip() or _resolve_operator_version_for_dsc()
        return _distributed_workloads_ready_condition_for_version(version)
    return cond
...
def dsc_disabled_reason_from_states(
    ...
    *,
    ...
    ready_type = _resolve_smoke_ready_condition(smoke_id, operator_version=operator_version)
...
def smoke_component_prereq_reason_from_states(
    ...
    *,
    ...
    version = (operator_version or "").strip() or _resolve_operator_version_for_dsc()
    ready_reason_key = _SMOKE_READY_CONDITION.get(smoke_id, "")
    if smoke_id == "distributed_workloads" and ready_reason:
        ready_reason_key = _distributed_workloads_ready_condition_for_version(version)
    disabled = dsc_disabled_reason_from_states(
        ...
        ready_reasons={ready_reason_key: ready_reason}
        if ready_reason
        else {},
    )
    if disabled:
    ...
    if smoke_id == "distributed_workloads":
        ready_type = _distributed_workloads_ready_condition_for_version(
            _resolve_operator_version_for_dsc(),
        )
