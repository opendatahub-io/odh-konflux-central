...
class TriggerContext:
    ...
    snapshot_yaml: str = ""
    snapshot_json: str = ""
    fbc_snapshot_meta: Mapping[str, Any] | None = None
...
def _rhoai_catalog_version(ctx: TriggerContext) -> str:
    return rhoai_catalog_version_from_fbc_source(
        fbc_image=_fbc_image_for_display(ctx),
        fbc_snapshot_meta=dict(ctx.fbc_snapshot_meta) if ctx.fbc_snapshot_meta else None,
        snapshot_json=(ctx.snapshot_json or "").strip(),
        fbc_component_name=_rhoai_fbc_name(ctx),
        resolved_app=ctx.resolved_app,
    )
...
TRIGGER_PARAMS: tuple[TriggerParam, ...] = (
    ...
    TriggerParam(
        ...
        infer=(
            (
                "FBC catalog line (snapshot metadata / image / Konflux app)",
                lambda c: True,
                _rhoai_catalog_version,
            ),
        ),
    ),
    TriggerParam(
    ...
    *,
    ...
    if snapshot_file is not None:
        try:
            ...
            snapshot_yaml = ""
    preview = getattr(runner, "_preview_snapshot_json", None)
    if callable(preview):
        try:
            snapshot_json = (preview(odh_overrides) or "").strip()
        except Exception:
            snapshot_json = ""
    ...
    return replace(
        ...
        fbc_snapshot_meta=getattr(runner, "_fbc_source_snapshot_meta", None),
