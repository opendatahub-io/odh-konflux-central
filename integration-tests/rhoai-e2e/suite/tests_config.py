"""Load rhoai-e2e-tests-config.yaml (PyYAML if available, else Mike Farah ``yq -o=json``)."""
...
class ArtifactUploadConfig:
    """Which files publish-results pushes to OCI and the browser subfolder name."""

    oci_subdir: str
    include_patterns: tuple[str, ...]


DEFAULT_ARTIFACT_UPLOAD = ArtifactUploadConfig(
    oci_subdir="test-payload-results",
    include_patterns=("*.xml", "*.log", "*.console.log"),
)
...
load_yaml_document = _load_yaml_document


def load_artifact_upload_config(path: Path) -> ArtifactUploadConfig:
    """Load ``artifactUpload`` from rhoai-e2e-tests-config.yaml (defaults when omitted)."""
    if not path.is_file():
        return DEFAULT_ARTIFACT_UPLOAD
    doc = _load_yaml_document(path)
    section = doc.get("artifactUpload")
    if not isinstance(section, dict):
        return DEFAULT_ARTIFACT_UPLOAD
    subdir = section.get("ociSubdir", DEFAULT_ARTIFACT_UPLOAD.oci_subdir)
    if not isinstance(subdir, str) or not subdir.strip():
        subdir = DEFAULT_ARTIFACT_UPLOAD.oci_subdir
    raw_patterns = section.get("includePatterns")
    patterns: list[str] = []
    ...
    if not patterns:
        patterns = list(DEFAULT_ARTIFACT_UPLOAD.include_patterns)
    return ArtifactUploadConfig(
        oci_subdir=subdir.strip().strip("/"),
