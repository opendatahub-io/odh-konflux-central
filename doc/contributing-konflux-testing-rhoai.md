...
Konflux/Tekton objects, OLM and operator vocabulary, and [**rhoai-e2e**](../integration-tests/rhoai-e2e/README.md) pipeline terms.
...
| <a id="bvt"></a>**BVT**                                 | Build Verification Test  -  basic post-install checks to confirm the operator deployed and core behavior works. The Konflux [rhoai-e2e](../integration-tests/rhoai-e2e/README.md) pipeline runs **opendatahub-tests** pytest only (same container image for BVT and smoke): `cluster_health` / `operator_health` when `TESTS` includes `bvt`, and per-component `-m smoke` under `tests/<component>/` when `TESTS` includes `smoke` (`COMPONENTS` + [rhoai-e2e-components-smoke.yaml](../integration-tests/rhoai-e2e/config/rhoai-e2e-components-smoke.yaml), [RHOAIENG-62891](https://redhat.atlassian.net/browse/RHOAIENG-62891)). No ods-ci execution from this pipeline.                                                                                                                                                                    |
...
| <a id="eaas"></a>**EaaS**                               | Environment as a Service  -  Konflux's on-demand ephemeral cluster provisioning ([docs](https://konflux.pages.redhat.com/docs/users/testing/cluster-provisioning.html#methods)). Other `integration-tests/` pipelines still use EaaS StepActions; **rhoai-e2e** uses [EPHC](#ephc) instead. |
| <a id="ephc"></a>**EPHC**                                 | Ephemeral Hosted Cluster in OpenShift CI (Prow)  -  rhoai-e2e `CLUSTER_SOURCE=EPHC` provisions via [`provision-ephemeral-cluster`](https://github.com/openshift/konflux-tasks/tree/main/tasks/provision-ephemeral-cluster/0.1). |
...
| ------- | ------------------ | -------------------- |
| **Manual** | You (`oc create` / `oc apply`, or [`rhoai_e2e.py`](../integration-tests/rhoai-e2e/rhoai_e2e.py)) | `test-pipelinerun.yaml`, dev iteration |
| **[ITS](#its)** | [Integration Service](#integration-service) after a build [Snapshot](#snapshot) | `its-rhoai-e2e-*.yaml` |
...
[rhoai-e2e](../integration-tests/rhoai-e2e/README.md) uses **[ITS](#its) + [Snapshot](#snapshot)** today, not [PAC](#pac).
...
## Snapshot-driven example (rhoai-e2e)

The **rhoai-e2e** integration under `integration-tests/rhoai-e2e/` exercises [Snapshot](#snapshot) + [ITS](#its) (not [PAC](#pac)): an [EPHC](#ephc) (Ephemeral Hosted Cluster in OpenShift CI (Prow)) is provisioned, the operator is installed from the snapshot catalog, then the cluster is destroyed. Alternatively, **`rhoai_e2e.py --external-kubeconfig`** skips EPHC and runs install/BVT/smoke against a kubeconfig you upload as a tenant Secret  -  see [External kubeconfig](../integration-tests/rhoai-e2e/README.md#external-kubeconfig). For triggers (including [`rhoai_e2e.py`](../integration-tests/rhoai-e2e/rhoai_e2e.py)), parameters, pipeline behavior, and glossary cross-links, see [integration-tests/rhoai-e2e/README.md](../integration-tests/rhoai-e2e/README.md) (**Triggering**). To stream logs or replay pruned runs from KubeArchive, use **`python3 integration-tests/rhoai-e2e/rhoai_e2e.py -w`** (see that README). Vocabulary is in [Terms and abbreviations](#terms-and-abbreviations) above.
